...
async function validateCouponServerSide(
  ...
  if (coupon.max_uses && (coupon.used_count || 0) >= (coupon.max_uses as number)) {
    return { discount: 0, couponId: null, couponCode: null };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.quantity <= 0 || item.quantity > maxPerOrder) {
        throw new Error(`Quantidade inválida para ${ticketType.name}`);
      }

      // Reserva ATÔMICA: trava a linha, confere disponibilidade e já
      // incrementa quantity_sold numa única operação -- sem essa trava, duas
      // compras simultâneas poderiam vender o mesmo último ingresso duas vezes.
    ...
    // Cupom: revalidado do zero no servidor -- nunca confia em desconto
    // calculado no navegador. Mesma regra que já existe no PIX.
    const { discount, couponId, couponCode } = await validateCouponServerSide(
      supabase, coupon_code, event_id, totalAmount
    );
    const subtotalAfterDiscount = Math.max(totalAmount - discount, 0);
    ...
    const totalWithFee = subtotalAfterDiscount + serviceFee + protectionFee;
    ...
    // Se o Mercado Pago já respondeu "rejeitado" na hora, devolve a reserva
    // também -- só fica reservado de verdade quem está aprovado ou em análise.
